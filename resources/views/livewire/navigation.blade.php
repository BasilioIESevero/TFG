<header class="bg-trueGray-700 sticky z-50 top-0" style="z-index: 900" x-data="dropdown()">
    <div class="container-menu flex items-center h-16 justify-between md:justify-start">
        <a id="category" :class="{'bg-opacity-100 text-pink-500': open}"
           x-on:click="show()"
    ...
    <nav id="navigation-menu"
            ...
            <div x-on:click.away="close()" class="grid grid-cols-4 h-full relative">
                <ul class="bg-white">
                    @foreach($categories as $category)
                        <li dusk="category_{{$category->id}}" class="navigation-link text-trueGray-500 hover:bg-pink-500 hover:text-white">
        ...
        <div class="bg-white h-full overflow-y-auto">
            ...
            <ul class="bg-white">
                @foreach($categories as $category)
                    <li class="text-trueGray-500 hover:bg-pink-500 hover:text-white">
                        <a href="{{ route('categories.show', $category) }}" class="py-2 px-4 text-sm flex items-center">
                            <span class="flex justify-center w-9">
                                {!! $category->icon !!}
                            </span>
            ...
            @auth
                <a href="{{ route('profile.show') }}" class="py-2 px-4 text-sm flex items-center text-trueGray-500 hover:bg-pink-500
                hover:text-white">
                <span class="flex justify-center w-9">
                ...
                <a href=""
                    ...
                    class="py-2 px-4 text-sm flex items-center text-trueGray-500 hover:bg-pink-500 hover:text-white">
                    <span class="flex justify-center w-9">
            ...
            @else
                <a href="{{ route('login') }}" class="py-2 px-4 text-sm flex items-center text-trueGray-500 hover:bg-pink-500
                    hover:text-white">
                    <span class="flex justify-center w-9">
                ...
                <a href="{{ route('register') }}" class="py-2 px-4 text-sm flex items-center text-trueGray-500 hover:bg-pink-500
                hover:text-white">
                <span class="flex justify-center w-9">
